...
class TestVersion(unittest.TestCase):
    ...
    def test_version_incremented(self):
        ...
        pypi_version, release_date = self.get_latest_pypi_version_and_date("openmsitoolbox")
        try:
            current_version = parse(openmsitoolbox.__version__)
        except InvalidVersion as exc:
            raise ValueError(f"Version string {openmsitoolbox.__version__} is not valid!") from exc
